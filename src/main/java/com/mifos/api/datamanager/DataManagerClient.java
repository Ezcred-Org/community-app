...
public class DataManagerClient {
    ...
    public DataManagerClient(
      ...
      DatabaseHelperClient databaseHelperClient, PrefManager prefManager
    ) {
     ...
     * @return SaveResponse is the Retrofit 2 response
     */
    public Observable<GenericResponse> uploadClientImage(int id, MultipartBody.Part file) {
    ...
    public Observable<GenericResponse> createMifosClient(final ClientPayload clientPayload) {
        return mBaseApiManager.getClientsApi().createMifosClient(clientPayload);
    }
         ...
         * @return Client
         *  @see DataManagerClient#createMifosClient(ClientPayload)
         */
        @Deprecated
     ...
     * @return List<Identifier>
     */
    public Observable<Identifier> getClientIdentifier(long clientId, final long documentId) {
        return mBaseApiManager
          .getClientsApi()
          .getClientIdentifiers(clientId)
          .flatMap(new Func1<List<Identifier>, Observable<Identifier>>() {
              @Override
              public Observable<Identifier> call(List<Identifier> identifiers) {
                  return Observable.from(identifiers);
              }
          })
          .first(new Func1<Identifier, Boolean>() {
              @Override
              public Boolean call(Identifier identifier) {
                  return identifier.getDocumentType().getId() == documentId;
              }
          });
    }
    ...
    /**
     * This Method is, for fetching the Client Address Template.
     *
     * @return Address
     */
    public Observable<Address> getClientAddressTemplate() {
        return mBaseApiManager.getClientsApi().getClientAddressTemplate();
    }
     ...
     * @return Address
     */
    public Observable<List<Address>> getClientAddresses(long clientId) {
     ...
     * @return Address
     */
    public Observable<List<Address>> getClientAddressesByType(long clientId, List<Boolean> status, long type ) {
        return mBaseApiManager.getClientsApi().getClientAddress(clientId, status, type);
    }
